...
obs_names = ['dETdy','dNdy']

for name in obs_names:
	infile_name1  = name + '_sim_testpp.dat'
	infile_name2  = name + '_sim_testpp_MC.dat'
	infile_name3  = name + '_sim_testpp_SAT.dat'
	infile_name4  = name + '_sim_testpp_SAT_MC.dat'

	pdf_name  = name+'_K=16.pdf'
	...
	ax.plot(y_points, np.transpose(data1), 'b-', label=r'$PDF$', linewidth=1)
	ax.plot(y_points, np.transpose(data2), 'b--', label=r'$PDF\;MC$', linewidth=1)
	ax.plot(y_points, np.transpose(data3), 'r-', label=r'$PDF\;SAT$', linewidth=1)
	ax.plot(y_points, np.transpose(data4), 'r--', label=r'$PDF\;SAT\;MC$', linewidth=1)
	ax.minorticks_on()
