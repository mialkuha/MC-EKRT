...
for name in obs_name:
	...
	ax.plot(pt_points, np.transpose(data7), 'b-.', label=r'$pp\;MC\;nd$', linewidth=1)
	ax.plot(pt_points, np.transpose(data8), 'r-.', label=r'$AA\;MC\;nd$', linewidth=1)
	ax.plot(pt_points, np.transpose(data9), 'g-.', label=r'$sAA\;MC\;nd$', linewidth=1)
	ax.minorticks_on()
	...
	ax.set_xscale('log')
	plt.grid(visible=True, which='both', axis='both')
	ax.plot(pt_points, np.transpose(data10), 'b-', label=r'$pp$', linewidth=1)
	ax.plot(pt_points, np.transpose(data11), 'r-', label=r'$AA$', linewidth=1)
	ax.plot(pt_points, np.transpose(data12), 'g-', label=r'$sAA$', linewidth=1)
	ax.plot(pt_points, np.transpose(data13), 'b--', label=r'$pp\;nd$', linewidth=1)
	ax.plot(pt_points, np.transpose(data14), 'r--', label=r'$AA\;nd$', linewidth=1)
	ax.plot(pt_points, np.transpose(data15), 'g--', label=r'$sAA\;nd$', linewidth=1)
	ax.minorticks_on()
	ax.tick_params(axis='both', which='both', direction='in', top=True, right=True)
	plt.legend(loc='upper right', ncol=1, fontsize='x-large')
