...
public:
      ...
      c_A_func(linear_interpolator(std::vector<double>{0.0},std::vector<double>{0.0}))
    {
        ...
        //     this->output.open("cm1.csv");
        //     this->output << "TA,r" << std::endl;
        //     this->output1.open("c1.csv");
        //     this->output1 << "TA,r" << std::endl;
        ...
        //             double r = std::exp(c * sum_tppa);
        //             // if (c<-0.995 && c > -1.005)
        //             // {
        //             //     const std::lock_guard<std::mutex> lock(this->output_mutex);
        //             //     this->output << sum_tppa << ',' << r << std::endl;
        //             // }
        //             return r;
                    
        //             // if (c < 0 && sum_tppa > -1.0/(2.0*c))
        //             // {
        //             //     return -1.0/(4.0*c);
        //             // }
        //             // else
        //             // {
        //             //     return sum_tppa*(1+c*sum_tppa);
        //             // }
        //         }
        //         else
        //         {
        ...
        //         {
        //             double c = this->c_A_func.value_at(r);
        //             double r = std::exp(c * sum_tppb);
        //             // if (c>0.995 && c<1.005)
        //             // {
        //             //     const std::lock_guard<std::mutex> lock(this->output_mutex1);
        //             //     this->output1 << sum_tppb << ',' << r << std::endl;
        //             // }
        //             return r;
                    
        //             // if (c < 0 && sum_tppb > -1.0/(2.0*c))
        //             // {
        //             //     return -1.0/(4.0*c);
        //             // }
        //             // else
        //             // {
        //             //     return sum_tppb*(1+c*sum_tppb);
        //             // }
        //         }
        //         else
        //         {
    ...
    {
        auto [pdf, scale, is_neutron, sum_tpp] = *(std::tuple
        <
            const pdf_builder*,
            double,
            bool,
        ...
        ret -= f_ai_x1[0];
        // ret = (f_i_x1[1] - f_ai_x1[1] + f_i_x1[2] - f_ai_x1[2])/x;

        return ret;
    }

    auto integrate_pdfs(std::vector<nucleon> &nucleus, std::shared_ptr<Tpp_builder> Tpp, double scale) const -> std::tuple<double, std::vector<double> > const
    {
        double sum = 0.0;
        std::vector<double> nucleons;

        for (auto &A : nucleus)
        {
            auto sum_tpp = Tpp->calculate_sum_tpp(A, nucleus);
            ...
            <
                ...
                bool,
                double
            > params = std::make_tuple(this, scale, A.is_neutron, sum_tpp);

            gsl_function F;
            F.function = &f;
            F.params = &params;

            gsl_integration_qags(&F, 1e-9, 1, 0, 1e-4, 1000,
                                    w, &result, &error);

            nucleons.push_back(result);
        ...
        return std::make_tuple(sum, nucleons);
    };
