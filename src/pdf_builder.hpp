...
public:
          ...
          c_A_func(linear_interpolator(std::vector<double>{0.0}, std::vector<double>{0.0}))
    {
        ...
        else if (this->npdfs_spatial && this->snPDFs_new)
        {
            std::vector<double> Rs_as_vector, cs_as_vector;

            std::cout << "Calculating R_A - c_A table" << std::endl;
            auto [R_A_table, c_A_table] = Tpp->calculate_R_c_table_new(
                nuc_params,
                1e-5,
                verbose);
    ...
    auto make_pdfs(
        ...
        const bool &average,
        const bool &max) const noexcept -> std::tuple<std::array<double, 7>,
                                                      std::array<double, 7>,
                                                      std::array<double, 7>,
                                                      std::array<double, 7>>
    {
        ...
        if (this->projectile_with_npdfs)
        {
            ...
            {
                rd = this->rA_spatial(rd, sum_tppa);
                ru = this->rA_spatial(ru, sum_tppa);
                rds = this->rA_spatial(rds, sum_tppa);
                rus = this->rA_spatial(rus, sum_tppa);
                rs = this->rA_spatial(rs, sum_tppa);
                rc = this->rA_spatial(rc, sum_tppa);
                rb = this->rA_spatial(rb, sum_tppa);
                rt = this->rA_spatial(rt, sum_tppa);
                rg = this->rA_spatial(rg, sum_tppa);
            }
        ...
        if (this->target_with_npdfs)
        {
            ...
            if (!average && this->npdfs_spatial)
            {
                rd = this->rB_spatial(rd, sum_tppb);
                ru = this->rB_spatial(ru, sum_tppb);
                rds = this->rB_spatial(rds, sum_tppb);
                rus = this->rB_spatial(rus, sum_tppb);
                rs = this->rB_spatial(rs, sum_tppb);
                rc = this->rB_spatial(rc, sum_tppb);
                rb = this->rB_spatial(rb, sum_tppb);
                rt = this->rB_spatial(rt, sum_tppb);
                rg = this->rB_spatial(rg, sum_tppb);
            }
