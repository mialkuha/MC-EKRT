...
double ars::throw_one(std::mt19937 & random_generator, const bool & adaptive) noexcept
{
    do
    {
        try
...
std::vector<double> ars::throw_n(const uint & n, std::mt19937 & random_generator, const bool & adaptive) noexcept
{
    std::vector<double> vec({});
    vec.reserve(n); 
    for (uint i=0; i<n; i++)
    {
        vec.emplace_back(this->throw_one(random_generator, adaptive));
    }
    return vec;
}
