...
void collide_nuclei(std::vector<nucleon> &pro, std::vector<nucleon> &tar, std::vector<nn_coll> &binary_collisions, std::uniform_real_distribution<double> unirand, std::shared_ptr<std::mt19937> eng, const AA_collision_params &params, const bool &verbose) noexcept
{
    ...
    for (auto &A : pro)
    {
        for (auto &B : tar)
        {
            ...
            {
                //sigma_jet = sigma_jets.value_at(pow(newpair.getcr_sqrt_s(), 2));
                xsectval sigma_jet = 179.2; //TODO
...
int main()
{
    ...
    uint desired_N_events = 1000, AA_events = 0, nof_collisions = 0;
    ...
    linear_interpolator sigma_jets(xs, ys);
    ...
    do // while (AA_events < desired_N_events);
    {
        ...
        collide_nuclei(pro, tar, binary_collisions, unirand, eng, coll_params, verbose);
